...
# Install vep (latest version=115) in a new conda environment
conda create --name annotation_2026 bioconda::ensembl-vep -c conda-forge
conda activate annotation_2026

# Check if software is available and its version
conda list | grep ensembl-vep

# Install a species specific cache file+FASTA that corresponds to a certain genome assembly of the studied species - also cache file should be always compatible with the vep version installed 
vep_install -a cf -s ovis_aries -y ARS-UI_Ramb_v2.0 --CONVERT #For sheep (genome assembly: ARS-UI_Ramb_v2.0) - we convert cache to tabix format for faster access 
